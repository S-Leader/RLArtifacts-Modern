...
public class LootTables extends LootTableProvider {
    ...
    private void addSacrificialRewardLootTable() {
        addLootTable(ModLootTables.SACRIFICIAL_REWARD.getPath(), LootTable.lootTable()
                .withPool(LootPool.lootPool()
                        .name("main")
                        .setRolls(ConstantValue.exactly(1))
                        .add(item(Items.NETHER_STAR, 1))
                        .add(item(Items.DIAMOND, 29, 3, 8))
                        .add(item(Items.EXPERIENCE_BOTTLE, 20, 2, 6))
                )
        );
    }
        ...
        addLootTable("items/drinking_hat", LootTable.lootTable()
                .withPool(
                        LootPool.lootPool()
                                .name("main")
                                .setRolls(ConstantValue.exactly(1))
                                .add(item(ModItems.PLASTIC_DRINKING_HAT.get(), 1))
                )
        );
    }
