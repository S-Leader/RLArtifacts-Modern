...
public class ArmsModel extends HumanoidModel<LivingEntity> {
    ...
    public static ArmsModel createLegacyGloveModel(boolean hasSlimArms) {
        return new ArmsModel(RendererUtil.bakeLayer(ArtifactLayers.legacyGlove(hasSlimArms)));
    }
    ...
    /**
     * Uses the vanilla 64x64 player-skin UV layout used by RLArtifacts 1.12.
     * This lets the original Mechanical Glove wearable textures render without
     * repacking or redrawing them.
     */
    public static MeshDefinition createLegacySleevedArms(boolean hasSlimArms) {
        float armWidth = hasSlimArms ? 3 : 4;
        CubeListBuilder leftArm = CubeListBuilder.create()
                .texOffs(32, 48).mirror()
                .addBox(-armWidth / 2, -12, -2, armWidth, 12, 4, new CubeDeformation(0.01F))
                .texOffs(48, 48).mirror()
                .addBox(-armWidth / 2, -12, -2, armWidth, 12, 4, new CubeDeformation(0.26F));
