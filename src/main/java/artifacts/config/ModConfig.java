...
public class ModConfig extends PartitioningSerializer.GlobalData {
    ...
    public static final class Common implements ConfigData {
        ...
        @ConfigEntry.Gui.Tooltip(count = 2)
        @Comment("Chance for an unlooted vanilla chest in the Overworld to be replaced by a Mimic when first opened or broken")
        public double unlootedChestMimicChance = 0.05D;

        public double getUnlootedChestMimicChance() {
            return Math.max(0.0D, Math.min(1.0D, unlootedChestMimicChance));
        }
